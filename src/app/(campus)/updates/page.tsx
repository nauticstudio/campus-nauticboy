      ...
      {(!announcements || announcements.length === 0) ? (
        ...
        <div className="space-y-6">
          {announcements.map(item => (
            <div key={item.id} className="glass-card glass-card-hover rounded-[var(--radius)] p-6 md:p-8 space-y-4 relative group">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <span className="px-3 py-1 rounded-full text-xs font-extrabold border bg-coral-100 text-coral-700 border-coral-300">
                  {item.type}
                </span>
              ...
              <div className="space-y-2">
                <h2 className="text-xl font-extrabold text-ink-900 group-hover:text-primary transition-colors tracking-tight">
                  {item.title}
                </h2>
                <p className="text-sm font-medium text-ink-600 leading-relaxed max-w-3xl">
