...
import { getLibraryResources } from '@/lib/data/library'
...
export default async function DashboardPage() {
  ...
  // La Home del alumno es la biblioteca: busqueda, filtros, contenido.
  // La RLS ya filtra lo que el alumno puede ver.
  const { items: libraryItems, filters: libraryFilters } = await getLibraryResources()
    ...
    <div className="p-6 md:p-10 lg:p-12 max-w-7xl mx-auto mb-20">
      <header className="mb-8">
        ...
        <p className="mt-1 text-sm font-medium text-ink-400">
          Todo el material del campus: plugins, presets, samples, plantillas y software.
        </p>
      </header>

      <LibraryBrowser items={libraryItems} filters={libraryFilters} />
    </div>
  )
}
