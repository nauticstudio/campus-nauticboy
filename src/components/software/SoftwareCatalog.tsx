...
/**
 * Catálogo reutilizable del hub de software.
 *
 * Vive originalmente en `/software` (que ahora redirige a la categoría
 * Plugins de la Academia) y se embebe dentro de `/academy/plugins` para que
 * el alumno vea fabricantes y productos en contexto. Los enlaces de cada
 * producto siguen apuntando a `/software/[marca]/[producto]` (páginas que
 * se conservan).
 */
  ...
  return (
    <div className="space-y-12">
        ...
        <div className="relative rounded-[var(--radius)] overflow-hidden bg-ink-900 text-white shadow-[var(--shadow-hero)] border border-ink-700 group">
          ...
          <div className="relative z-20 p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="space-y-4 max-w-xl">
              ...
              <h2 className="font-display text-3xl md:text-5xl font-semibold tracking-quant text-white">
                {heroProduct.name}
              </h2>
      ...
      {manufacturers.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            {selectedManufacturer && (
              <Link href="/academy/plugins#catalog-grid" className="text-xs font-bold text-primary hover:text-primary transition-colors">
                Ver Todos
              </Link>
            )}
          </div>
      ...
      <div id="catalog-grid" className="space-y-6 scroll-mt-28">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <h2 className="font-display text-xl font-semibold tracking-quant text-ink-50">
              ...
              >
                  ...
                  <div className="relative h-44 rounded-xl overflow-hidden bg-ink-950 border border-ink-700/40">
                    {product.cover_image_url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <Cpu className="w-12 h-12 text-ink-400" />
                      </div>
                    )}
                    ...
                    <div className={`absolute top-3 px-2.5 py-1 rounded-lg bg-ink-950/80 backdrop-blur-md text-[10px] font-extrabold text-ink-200 uppercase tracking-wider border border-ink-700/50 ${(product.icon_url || product.logo_image_url) ? 'left-14' : 'left-3'}`}>
                      {product.manufacturer?.name}
                    </div>
                  </div>

                  <div>
                    <h3 className="font-display text-xl font-semibold text-ink-50 tracking-quant group-hover:text-coral-300 transition-colors">
                      {product.name}
                    </h3>
                ...
                <div className="pt-5 mt-5 border-t border-ink-700/40 flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-[11px] font-bold text-ink-400">
                    <CheckCircle2 className="w-3.5 h-3.5 text-coral-300" /> v{product.version}
                  </div>
